...
for node = 1:cfg.n_array
    for src=1:cfg.n_src
        for mic= 1:cfg.n_mic
           [X(:,:,mic,src,node),fbins,tbins] = spectrogram(xn(:,mic,node),cfg.music.window,cfg.music.n_overlap,cfg.music.n_fft,cfg.fs);   
        end
    end
end
cfg.n_src = 1;
...
for i = 1:cfg.n_array
    DOA(i,:) = binNarrowMusicDoa(squeeze(X(:,:,:,1,i)),cfg.res,cfg);
    DOA2(i,:) = binNarrowMusicDoa(squeeze(X(:,:,:,2,i)),cfg.res,cfg);
end

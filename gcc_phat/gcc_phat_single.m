function [tau,r12,lags] = gcc_phat_single(x,varargin)
...
validateattributes(x,{'double'},{'2d','nonempty','finite'},'gccphat','A');
[N,M] = size(x);
sigdatatypes.validateFrequency(fs,'gccphat','Fs',{'scalar'});
if refflag
    if iscolumn(xref)
        validateattributes(xref,{'double'},{'nonempty','finite','nrows',N},'gccphat','B');
    else
        validateattributes(xref,{'double'},{'nonempty','finite','size',[N M]},'gccphat','B');
    end
end
...
lags = lags/fs;
if refflag
    r12 = privgccphat(x,xref,NFFT,Ncorr);
else
    r12 = complex(zeros(Ncorr,M^2));
    for m = 1:M
